...
class AdsController extends CommonController {
    ...
    public function click($hash,$aid){
        ...
        $mo->execute('lock tables qqbtc_app_log write ,qqbtc_user_coin write');

        $rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $uid))->setDec($price_coin['name'], $Vip['price_num']);
        $rs[] = $mo->table('qqbtc_app_log')->add(
            array(
                'uid'     => $uid,
                'type'    => 'click_ads',
