...
class PoolController extends HomeController
{
	...
	public function index()
	{
		...
		if (IS_POST) {
			...
			$mo->execute('lock tables  qqbtc_user write , qqbtc_pool_log  write ,qqbtc_user_coin write');
			$rs = array();
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $user['id']))->setDec(C('rmb_mr'), $mum);
			$rs[] = $mo->table('qqbtc_pool_log')->add(array('userid' => $user['id'], 'coinname' => $pool['coinname'], 'name' => $pool['name'], 'ico' => $pool['ico'], 'price' => $pool['price'], 'num' => $input['num'], 'tian' => $pool['tian'], 'power' => $pool['power'], 'endtime' => time(), 'addtime' => time(), 'status' => 0));
	...
	public function startpool()
	{
		...
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables qqbtc_pool_log write');
			$rs = array();
			$rs[] = $mo->table('qqbtc_pool_log')->where(array('id' => $poolLog['id']))->save(array('endtime' => time(), 'status' => 1));
	...
	public function receiving()
	{
		...
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables qqbtc_user_coin write ,  qqbtc_pool_log  write ');
			$rs = array();
			$num = round($poolLog['num'] * C('pool_suan') * $poolLog['power'], 6);
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $user['id']))->setInc($poolLog['coinname'], $num);
			$rs[] = $mo->table('qqbtc_pool_log')->where(array('id' => $poolLog['id']))->save(array('use' => $poolLog['use'] + 1, 'endtime' => time()));

			if ($poolLog['tian'] <= $poolLog['use'] + 1) {
				$rs[] = $mo->table('qqbtc_pool_log')->where(array('id' => $poolLog['id']))->save(array('status' => 2));
			}
