...
class ChartController extends HomeController
{
	...
	{
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from qqbtc_trade  where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit 100;');
		$data = '';
	...
	{
		$mo = M();
		$sell = $mo->query('select id,price,sum(num-deal)as nums from qqbtc_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit 100;');
		$data = '';
