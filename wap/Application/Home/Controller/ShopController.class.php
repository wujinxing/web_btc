...
class ShopController extends HomeController
{
	...
	public function buyShop($id, $num, $paypassword, $type, $goods)
	{
		...
		$mo->execute('lock tables qqbtc_user_coin write,qqbtc_shop write,qqbtc_shop_log write');
		$rs = array();
		$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => userid()))->setDec($type, $xuyao);
		$rs[] = $mo->table('qqbtc_shop')->where(array('id' => $Shop['id']))->save(array(
			'deal' => array('exp', 'deal+' . $num),
			'num'  => array('exp', 'num-' . $num)
			));

		if ($Shop['num'] - $num <= 0) {
			$rs[] = $mo->table('qqbtc_shop')->where(array('id' => $Shop['id']))->save(array('status' => 0));
		}

		$rs[] = $mo->table('qqbtc_shop_log')->add(array('userid' => userid(), 'shopid' => $Shop['id'], 'price' => $Shop['price'], 'coinname' => $type, 'xuyao' => $xuyao, 'num' => $num, 'mum' => $mum, 'addr' => $my_goods['truename'] . '|' . $my_goods['moble'] . '|' . $my_goods['addr'], 'addtime' => time(), 'status' => 0));
	...
	public function install()
	{
		...
		if (in_array('shop', $game_arr)) {
			...
			if (!isset($tableMap['qqbtc_shop_log'])) {
			...
			if (!isset($tableMap['qqbtc_shop_type'])) {
			...
			if (!isset($tableMap['qqbtc_shop_coin'])) {
