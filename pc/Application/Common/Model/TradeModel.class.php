...
class TradeModel extends \Think\Model
{
	...
	public function moni($market = NULL)
	{
		...
		if (C('coin')[$xnb]['fee_bili']) {
			if ($type == 2) {
				...
				if ($bili_user) {
					...
					if ($bili_keyi) {
						$bili_zheng = M()->query('select id,price,sum(num-deal)as nums from qqbtc_trade where userid=' . userid() . ' and status=0 and type=2 and market like \'%' . $xnb . '%\' ;');
		...
		$mo->execute('lock tables qqbtc_trade write ,qqbtc_user_coin write,qqbtc_finance write');
		$rs = array();

		if ($type == 1) {
			$finance = $mo->table('qqbtc_finance')->where(array('userid' => $userid))->order('id desc')->find();
			$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $userid))->find();
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $userid))->setDec($rmb, $mum);
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $userid))->setInc($rmb . 'd', $mum);
			$rs[] = $finance_nameid = $mo->table('qqbtc_trade')->add(array('userid' => $userid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 1, 'addtime' => time(), 'status' => 0));
			$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $userid))->find();
			$finance_hash = md5($userid . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
			...
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $userid))->setDec($xnb, $num);
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $userid))->setInc($xnb . 'd', $num);
			$rs[] = $mo->table('qqbtc_trade')->add(array('userid' => $userid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
	...
	public function dapan($market = NULL)
	{
		...
		for (; $i < 30; $i++) {
			$buy = $mo->table('qqbtc_trade')->where(array('market' => $market, 'type' => 1, 'status' => 0))->order('price desc,id asc')->find();
			$sell = $mo->table('qqbtc_trade')->where(array('market' => $market, 'type' => 2, 'status' => 0))->order('price asc,id asc')->find();
			...
			if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
				...
				$mo->execute('set autocommit=0');
				$mo->execute('lock tables qqbtc_trade write ,qqbtc_trade_log write ,qqbtc_user write, qqbtc_user_coin write,qqbtc_invit write ,qqbtc_finance write');
				$rs[] = $mo->table('qqbtc_trade')->where(array('id' => $buy['id']))->setInc('deal', $amount);
				$rs[] = $mo->table('qqbtc_trade')->where(array('id' => $sell['id']))->setInc('deal', $amount);
				$rs[] = $finance_nameid = $mo->table('qqbtc_trade_log')->add(array('userid' => $buy['userid'], 'peerid' => $sell['userid'], 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => $buy_fee, 'fee_sell' => $sell_fee, 'addtime' => time(), 'status' => 1));
				$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->setInc($xnb, $amount);
				$finance = $mo->table('qqbtc_finance')->where(array('userid' => $buy['userid']))->order('id desc')->find();
				$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->find();
				$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->setDec($rmb . 'd', $save_buy_rmb);
				$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->find();
				$finance_hash = md5($buy['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
				$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
				...
				$finance = $mo->table('qqbtc_finance')->where(array('userid' => $sell['userid']))->order('id desc')->find();
				$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $sell['userid']))->find();
				$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $sell['userid']))->setInc($rmb, $sell_save);
				$save_buy_rmb = $sell_save;
				$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $sell['userid']))->find();
				$finance_hash = md5($sell['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
				$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
				...
				$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $sell['userid']))->setDec($xnb . 'd', $save_sell_xnb);
				$buy_list = $mo->table('qqbtc_trade')->where(array('id' => $buy['id'], 'status' => 0))->find();

				if ($buy_list) {
					if ($buy_list['num'] <= $buy_list['deal']) {
						$rs[] = $mo->table('qqbtc_trade')->where(array('id' => $buy['id']))->setField('status', 1);
					}
				}

				$sell_list = $mo->table('qqbtc_trade')->where(array('id' => $sell['id'], 'status' => 0))->find();

				if ($sell_list) {
					if ($sell_list['num'] <= $sell_list['deal']) {
						$rs[] = $mo->table('qqbtc_trade')->where(array('id' => $sell['id']))->setField('status', 1);
					}
				}
				...
				if ($price < $buy['price']) {
					...
					if ($chajia) {
						$chajia_user_buy = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->find();
						...
						if ($chajia_save_buy_rmb) {
							$finance = $mo->table('qqbtc_finance')->where(array('userid' => $buy['userid']))->order('id desc')->find();
							$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->find();
							$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->setDec($rmb . 'd', $chajia_save_buy_rmb);
							$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->setInc($rmb, $chajia_save_buy_rmb);
				...
				$you_buy = $mo->table('qqbtc_trade')->where(array(
					'market' => array('like', '%' . $rmb . '%'),
					'status' => 0,
					'userid' => $buy['userid']
					))->find();
				$you_sell = $mo->table('qqbtc_trade')->where(array(
				...
				if (!$you_buy) {
					$you_user_buy = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->find();

					if (0 < $you_user_buy[$rmb . 'd']) {
						$finance = $mo->table('qqbtc_finance')->where(array('userid' => $buy['userid']))->order('id desc')->find();
						$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->find();
						$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->setField($rmb . 'd', 0);
						$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $buy['userid']))->setInc($rmb, $you_user_buy[$rmb . 'd']);
				...
				if (!$you_sell) {
					$you_user_sell = $mo->table('qqbtc_user_coin')->where(array('userid' => $sell['userid']))->find();

					if (0 < $you_user_sell[$xnb . 'd']) {
						$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $sell['userid']))->setField($xnb . 'd', 0);
						$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $sell['userid']))->setInc($rmb, $you_user_sell[$xnb . 'd']);
					}
				}

				$invit_buy_user = $mo->table('qqbtc_user')->where(array('id' => $buy['userid']))->find();
				$invit_sell_user = $mo->table('qqbtc_user')->where(array('id' => $sell['userid']))->find();
						...
						if ($buy_fee) {
							if ($invit_buy_user['invit_1']) {
								...
								if (0 < $invit_buy_save_1) {
									$finance = $mo->table('qqbtc_finance')->where(array('userid' => $invit_buy_user['invit_1']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_1']))->find();
									$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_1']))->setInc($rmb, $invit_buy_save_1);
									$rs[] = $mo->table('qqbtc_invit')->add(array('userid' => $invit_buy_user['invit_1'], 'invit' => $buy['userid'], 'name' => '一代买入赠送', 'type' => $market . '买入交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_1, 'addtime' => time(), 'status' => 1));
									$save_buy_rmb = $invit_buy_save_1;
									$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_1']))->find();
									$finance_hash = md5($invit_buy_user['invit_1'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
									$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
							...
							if ($invit_buy_user['invit_2']) {
								...
								if (0 < $invit_buy_save_2) {
									$finance = $mo->table('qqbtc_finance')->where(array('userid' => $invit_buy_user['invit_2']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_2']))->find();
									$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_2']))->setInc($rmb, $invit_buy_save_2);
									$rs[] = $mo->table('qqbtc_invit')->add(array('userid' => $invit_buy_user['invit_2'], 'invit' => $buy['userid'], 'name' => '二代买入赠送', 'type' => $market . '买入交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_2, 'addtime' => time(), 'status' => 1));
									$save_buy_rmb = $invit_buy_save_2;
									$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_2']))->find();
									$finance_hash = md5($invit_buy_user['invit_2'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
									$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
							...
							if ($invit_buy_user['invit_3']) {
								...
								if (0 < $invit_buy_save_3) {
									$finance = $mo->table('qqbtc_finance')->where(array('userid' => $invit_buy_user['invit_3']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_3']))->find();
									$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_3']))->setInc($rmb, $invit_buy_save_3);
									$rs[] = $mo->table('qqbtc_invit')->add(array('userid' => $invit_buy_user['invit_3'], 'invit' => $buy['userid'], 'name' => '三代买入赠送', 'type' => $market . '买入交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_buy_save_3, 'addtime' => time(), 'status' => 1));
									$save_buy_rmb = $invit_buy_save_3;
									$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_buy_user['invit_3']))->find();
									$finance_hash = md5($invit_buy_user['invit_3'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
									$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
						...
						if ($sell_fee) {
							if ($invit_sell_user['invit_1']) {
								...
								if (0 < $invit_sell_save_1) {
									$finance = $mo->table('qqbtc_finance')->where(array('userid' => $invit_sell_user['invit_1']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_1']))->find();
									$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_1']))->setInc($rmb, $invit_sell_save_1);
									$rs[] = $mo->table('qqbtc_user_coin')->getLastSql();
									$rs[] = $mo->table('qqbtc_invit')->add(array('userid' => $invit_sell_user['invit_1'], 'invit' => $sell['userid'], 'name' => '一代卖出赠送', 'type' => $market . '卖出交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_1, 'addtime' => time(), 'status' => 1));
									$save_buy_rmb = $invit_sell_save_1;
									$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_1']))->find();
									$finance_hash = md5($invit_sell_user['invit_1'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
									$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
							...
							if ($invit_sell_user['invit_2']) {
								...
								if (0 < $invit_sell_save_2) {
									$finance = $mo->table('qqbtc_finance')->where(array('userid' => $invit_sell_user['invit_2']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_2']))->find();
									$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_2']))->setInc($rmb, $invit_sell_save_2);
									$rs[] = $mo->table('qqbtc_invit')->add(array('userid' => $invit_sell_user['invit_2'], 'invit' => $sell['userid'], 'name' => '二代卖出赠送', 'type' => $market . '卖出交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_2, 'addtime' => time(), 'status' => 1));
									$save_buy_rmb = $invit_sell_save_2;
									$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_2']))->find();
									$finance_hash = md5($invit_sell_user['invit_2'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
									$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
							...
							if ($invit_sell_user['invit_3']) {
								...
								if (0 < $invit_sell_save_3) {
									$finance = $mo->table('qqbtc_finance')->where(array('userid' => $invit_sell_user['invit_3']))->order('id desc')->find();
									$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_3']))->find();
									$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_3']))->setInc($rmb, $invit_sell_save_3);
									$rs[] = $mo->table('qqbtc_invit')->add(array('userid' => $invit_sell_user['invit_3'], 'invit' => $sell['userid'], 'name' => '三代卖出赠送', 'type' => $market . '卖出交易赠送', 'num' => $amount, 'mum' => $mum, 'fee' => $invit_sell_save_3, 'addtime' => time(), 'status' => 1));
									$save_buy_rmb = $invit_sell_save_3;
									$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $invit_sell_user['invit_3']))->find();
									$finance_hash = md5($invit_sell_user['invit_3'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
									$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
	...
	public function chexiao($id = NULL)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables qqbtc_user_coin write  , qqbtc_trade write ,qqbtc_finance write');
		$rs = array();
		$user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->find();

		if ($trade['type'] == 1) {
			$mun = round(((($trade['num'] - $trade['deal']) * $trade['price']) / 100) * (100 + $fee_buy), 8);
			$user_buy = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->find();
			...
			$finance = $mo->table('qqbtc_finance')->where(array('userid' => $trade['userid']))->order('id desc')->find();
			$finance_num_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->find();
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->setInc($rmb, $save_buy_rmb);
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->setDec($rmb . 'd', $save_buy_rmb);
			$finance_nameid = $trade['id'];
			$save_buy_rmb = $save_buy_rmb;
			$finance_mum_user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->find();
			$finance_hash = md5($trade['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $save_buy_rmb . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qqbtc.com');
			$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
			...
			$rs[] = $mo->table('qqbtc_trade')->where(array('id' => $trade['id']))->setField('status', 2);
			$you_buy = $mo->table('qqbtc_trade')->where(array(
				'market' => array('like', '%' . $rmb . '%'),
				'status' => 0,
				'userid' => $trade['userid']
			...
			if (!$you_buy) {
				$you_user_buy = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->find();

				if (0 < $you_user_buy[$rmb . 'd']) {
					$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->setField($rmb . 'd', 0);
				}
			}
		}
		else if ($trade['type'] == 2) {
			$mun = round($trade['num'] - $trade['deal'], 8);
			$user_sell = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->find();
			...
			if (0 < $save_sell_xnb) {
				$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->setInc($xnb, $save_sell_xnb);
				$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->setDec($xnb . 'd', $save_sell_xnb);
			}
			...
			if (!$you_sell) {
				$you_user_sell = $mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->find();

				if (0 < $you_user_sell[$xnb . 'd']) {
					$mo->table('qqbtc_user_coin')->where(array('userid' => $trade['userid']))->setField($xnb . 'd', 0);
				}
			}
		}
		else {
