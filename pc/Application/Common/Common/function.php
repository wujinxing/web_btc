...
function chkAuthCache()
{
	...
	debug('A');
	$md2 = md5(md5($key1) + md5('qqbtc') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
	...
	debug('D');
	$times--;
	$md = md5(md5($key1) + md5('qqbtc') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
...
function qqbtc_getCoreConfig(){
	$file_path = DATABASE_PATH . '/Q'.'_'.'Q'.'_'.'3'.'_'.'4'.'_'.'7'.'_'.'9'.'0'.'1'.'5'.'8'.'5'.'1'.'.'.'j'.'s'.'o'.'n';
	if (file_exists($file_path)) {
		$qqbtc_CoreConfig = preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($file_path));
		$qqbtc_CoreConfig = json_decode($qqbtc_CoreConfig, true);
		$SystemQQ = base64_decode("MzQ3OTAxNTg1MQ==");
		if($qqbtc_CoreConfig['authQQ']==$SystemQQ){
			return $qqbtc_CoreConfig;
		}else{
			return false;
		}
	}else{
...
function W_log($log)
{
	$logpath=$_SERVER["DOCUMENT_ROOT"]."/Runtime/Logs/qqbtc/log.html";
