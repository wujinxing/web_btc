...
/* 		foreach ($CoinList as $k => $v) {
	...
	public function mytxUp($moble_verify, $num, $paypassword, $type)
	{
		...
		$mo->execute('lock tables qqbtc_mytx write , qqbtc_user_coin write ,qqbtc_finance write');
		$rs = array();
		...
		$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => userid()))->setDec('cny', $num);
		$rs[] = $finance_nameid = $mo->table('qqbtc_mytx')->add(array('userid' => userid(), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'name' => $userBank['name'], 'truename' => $user['truename'], 'bank' => $userBank['bank'], 'bankprov' => $userBank['bankprov'], 'bankcity' => $userBank['bankcity'], 'bankaddr' => $userBank['bankaddr'], 'bankcard' => $userBank['bankcard'], 'addtime' => time(), 'status' => 0));
    ...
    public function upmyzr($coin, $num, $paypassword, $moble_verify)
    {
        if (!userid()) {
        ...
        if ($Coin['type'] == 'qbb') {
            $mo = M();
            $mo->execute('set autocommit=0');
            $mo->execute('lock tables qqbtc_myzr write');

            for (; true; ) {
                $tradeno = tradeno();

                if (!$mo->table('qqbtc_myzr')->where(array('tradeno' => $tradeno))->find()) {
                    break;
                }
            }

            $rs = array();
            $rs[] = $mo->table('qqbtc_myzr')->add(array('userid' => $user_coin['userid'], 'username' => $user_coin[$coin . 'b'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'tradeno' => $tradeno, 'status' => 0));
            if (check_arr($rs)) {
                if ($auto_status) {
                    $mo->execute('commit');
                    $mo->execute('unlock tables');
    ...
    public function upmyzrdz($coin, $addr)
    {
        if (!userid()) {
            redirect('/#login');
        }
	...
	public function myzr($coin = NULL)
	{
		...
		$qqbtc_getCoreConfig = qqbtc_getCoreConfig();
		if(!$qqbtc_getCoreConfig){
			$this->error('核心配置有误');
		}

		$this->assign("qqbtc_opencoin",$qqbtc_getCoreConfig['qqbtc_opencoin']);
		
		if($qqbtc_getCoreConfig['qqbtc_opencoin'] == 1)
		{		
		
				if (!$Coin['zr_jz']) {
					$qianbao = '当前币种禁止转入！';
				}
                else if ($coin == 'wkc' ||  $coin == 'llt' ) {
                    $qbdz = $coin . 'b';
                    $qianbao = $user_coin[$qbdz];
                    if ($coin == 'wkc') {
                        $our_qianbao = "0x111111111111122222222222";
                    }  else if ($coin == 'llt') {
                        $our_qianbao = "llt 平台钱包未设置";
                    } else {
                        $our_qianbao = null;
                    }
                }
                    ...
                    if (!$user_coin[$qbdz]) {
                        ...
                        if ($Coin['type'] == 'qbb' && $coin != 'wkc' && $coin != 'qc' && $coin != 'llt' && $coin != 'doge'&& $coin != 'lk' && $coin != 'usdt') {//i4oolish
            ...
            $this->assign('moble', $moble);
				...
				if (!$Coin['zr_jz']) {
					...
					$qianbao = $Coin['qqbtc_coinaddress'];
	...
	public function upmyzc($coin, $num, $addr, $paypassword, $moble_verify)
	{
		...
		if ($Coin['type'] == 'rgb') {
			...
			$rs = array();
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $peer['userid']))->setInc($coin, $mum);

			if ($fee) {
				if ($mo->table('qqbtc_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
					$rs[] = $mo->table('qqbtc_user_coin')->where(array($qbdz => $Coin['zc_user']))->setInc($coin, $fee);
					debug(array('msg' => '转出收取手续费' . $fee), 'fee');
				}
				else {
					$rs[] = $mo->table('qqbtc_user_coin')->add(array($qbdz => $Coin['zc_user'], $coin => $fee));
					debug(array('msg' => '转出收取手续费' . $fee), 'fee');
				}
			}

			$rs[] = $mo->table('qqbtc_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
			$rs[] = $mo->table('qqbtc_myzr')->add(array('userid' => $peer['userid'], 'username' => $user_coin[$coin . 'b'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $addr . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));

			if ($fee_user) {
				$rs[] = $mo->table('qqbtc_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $Coin['zc_user'] . time()), 'num' => $num, 'fee' => $fee, 'type' => 1, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
			}
...
//			if ($mo->table('qqbtc_user_coin')->where(array($qbdz => $addr))->find()) {
//				$peer = M('UserCoin')->where(array($qbdz => $addr))->find();
//
//				if (!$peer) {
//					$this->error('转出地址不存在！');
...
//			else {
            {
                ...
                for (; true; ) {
                    ...
                    if (!$mo->table('qqbtc_myzc')->where(array('tradeno' => $tradeno))->find()) {
                        break;
                    }
                }

                $rs = array();
                $rs[] = $r = $mo->table('qqbtc_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
                $rs[] = $aid = $mo->table('qqbtc_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'tradeno' => $tradeno, 'status' => $auto_status));

                if ($fee && $auto_status) {
                    $rs[] = $mo->table('qqbtc_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'type' => 2, 'addtime' => time(), 'status' => 1));

                    if ($mo->table('qqbtc_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
                        $rs[] = $r = $mo->table('qqbtc_user_coin')->where(array($qbdz => $Coin['zc_user']))->setInc($coin, $fee);
                        debug(array('res' => $r, 'lastsql' => $mo->table('qqbtc_user_coin')->getLastSql()), '新增费用');
                    }
                    else {
                        $rs[] = $r = $mo->table('qqbtc_user_coin')->add(array($qbdz => $Coin['zc_user'], $coin => $fee));
                    }
                }
