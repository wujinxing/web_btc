...
class PayController extends HomeController
{
	public function index()
	{
		if (IS_POST) {
			if (isset($_POST['alipay'])) {
				...
				$mo->execute('lock tables qqbtc_user_coin write,qqbtc_mycz write,qqbtc_finance write');
				$rs = array();
				...
				$rs[] = $mo->table('qqbtc_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
	...
	public function movepay()
	{
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables qqbtc_user_coin write,qqbtc_mycz write');
			$rs = array();
			$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
			$rs[] = $mo->table('qqbtc_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
