...
class TradeController extends HomeController
{
	public function index($market = NULL)
	{
		...
		$market_time_qqbtc = C('market')[$market]['begintrade']."-".C('market')[$market]['endtrade'];
		
		
		
		$this->assign('market_time', $market_time_qqbtc);
	...
	public function upTrade($paypassword = NULL, $market = NULL, $price, $num, $type)
	{
		...
		$rs = array();
		$user_coin = $mo->table('qqbtc_user_coin')->where(array('userid' => userid()))->find();
	...
	public function matchingTrade($market = NULL)
	{
		...
		for (; true; ) {
			$buy = $mo->table('qqbtc_trade')->where(array('market' => $market, 'type' => 1,'userid' => array('gt',0), 'status' => 0))->order('price desc,id asc')->find();
			$sell = $mo->table('qqbtc_trade')->where(array('market' => $market, 'type' => 2,'userid' => array('gt',0), 'status' => 0))->order('price asc,id asc')->find();
	...
	public function matchingAutoTrade($market = NULL)
	{
		...
		for (; true; ) {
			$buy = $mo->table('qqbtc_trade')->where(array('market' => $market, 'type' => 1, 'userid'=>0,'status' => 0))->order('price desc,id asc')->find();
			$sell = $mo->table('qqbtc_trade')->where(array('market' => $market, 'type' => 2, 'userid'=>0,'status' => 0))->order('price asc,id asc')->find();
			...
			if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
				...
				$mo->execute('set autocommit=0');
				$mo->execute('lock tables qqbtc_trade write ,qqbtc_trade_log write ');
				
				$rs[] = $mo->table('qqbtc_trade')->where(array('id' => $buy['id']))->setInc('deal', $amount);
				$rs[] = $mo->table('qqbtc_trade')->where(array('id' => $sell['id']))->setInc('deal', $amount);
				
				$rs[] = $mo->table('qqbtc_trade_log')->add(array('userid' => 0, 'peerid' => 0, 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => 0, 'fee_sell' => 0, 'addtime' => time(), 'status' => 1));
		...
		mlog("duqunew_trade_".$new_trade_qqbtc);
		
		if ($new_trade_qqbtc) {
			
			mlog("wojinlail".$new_trade_qqbtc);
