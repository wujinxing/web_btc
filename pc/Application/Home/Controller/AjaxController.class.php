...
class AjaxController extends HomeController
{
	...
	public function top_coin_menu($ajax = 'json')
	{
		...
		$data = (APP_DEBUG ? null : S('qqbtc_getTopCoinMenu'));
		...
		if (!$data) {
			...
			foreach($qqbtc_getCoreConfig['qqbtc_indexcat'] as $k=>$v){
				$data[$k][title] = $v;
			}
...
/* 				if (C('market')[$v['name'] . '_cny']['new_price']) {
	...
	{
		//$data = (APP_DEBUG ? null : S('qqbtc_allcoin'));
		
		
		$qqbtc_data=array();
		
		$qqbtc_data['info']="数据异常";
		$qqbtc_data['status']=0;
		$qqbtc_data['url']="";
		...
		if (!$data) {
			...
			foreach (C('market') as $k => $v) {
				...
				if($id==1){
					if(stristr($k,'_cny')){
						$qqbtc_data['url'][$k][0] = $v['title'];
						$qqbtc_data['url'][$k][1] = round($v['new_price'], $v['round']);
						$qqbtc_data['url'][$k][2] = round($v['buy_price'], $v['round']);
						$qqbtc_data['url'][$k][3] = round($v['sell_price'], $v['round']);
						$qqbtc_data['url'][$k][4] = isset($themarketLogs[$k]) ? $themarketLogs[$k] : 0;//round($v['volume'] * $v['new_price'], 2) * 1;
						$qqbtc_data['url'][$k][5] = '';
						$qqbtc_data['url'][$k][6] = round($v['volume'], 2) * 1;
						$qqbtc_data['url'][$k][7] = round($v['change'], 2);
						$qqbtc_data['url'][$k][8] = $v['name'];
						$qqbtc_data['url'][$k][9] = $v['xnbimg'];
						$qqbtc_data['url'][$k][10] = '';
					}
				}
				if($id==2){
...
/*         if ($marketLogs) {
	...
	public function getDepth($market = NULL, $trade_moshi = 1, $ajax = 'json')
	{
		...
		if(!$qqbtc_getCoreConfig){
			...
			$qqbtc_putong = $qqbtc_getCoreConfig['qqbtc_userTradeNum'];
			$qqbtc_teshu = $qqbtc_getCoreConfig['qqbtc_specialUserTradeNum'];
		}
		...
		if (!$data_getDepth[$market][$trade_moshi]) {
			...
			$mo = M();
			if ($trade_moshi == 1) {
				$buy = $mo->query('select id,price,sum(num-deal)as nums from qqbtc_trade where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit ' . $limt . ';');
				$sell = array_reverse($mo->query('select id,price,sum(num-deal)as nums from qqbtc_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit ' . $limt . ';'));
			}
	...
	public function getEntrustAndUsercoin($market = NULL, $ajax = 'json')
	{
		...
		$result = M()->query('select id,price,num,deal,mum,type,fee,status,addtime from qqbtc_trade where status=0 and market=\'' . $market . '\' and userid=' . userid() . ' order by id desc limit 10;');
