...
class ChartController extends HomeController
{
	public function getJsonData($market = NULL, $ajax = 'json')
	{
		if ($market) {
			...
			if(!$qqbtc_getCoreConfig){
				...
				$qqbtc_putong = $qqbtc_getCoreConfig['qqbtc_userTradeDetailNum'];
				$qqbtc_teshu = $qqbtc_getCoreConfig['qqbtc_specialUserTradeDetailNum'];
			}
	...
	protected function getTradeBuy($market,$limit)
	{
		...
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from qqbtc_trade  where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit '.$limit);
		$data = '';
	...
	protected function getTradeSell($market,$limit)
	{
		$limit = intval($limit);
		$mo = M();
		$sell = $mo->query('select id,price,sum(num-deal)as nums from qqbtc_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit '.$limit);
		$data = '';
	...
	public function getMarketNewKqqbtcJson()
	{
		// TODO: SEPARATE
		$input = I('get.');
		$market = (is_array(C('market')[$input['market']]) ? trim($input['market']) : C('market_mr'));
