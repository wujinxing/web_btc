...
class LoginController extends HomeController
{
	...
	public function real($moble, $verify)
	{
		...
		$content = '您正在进行注册本站会员，您的验证码是' . $code;
	...
	public function submit($username="", $password="",$moble="", $verify = NULL)
	{
		...
		$rs[] = $mo->table('qqbtc_user')->where(array('id' => $user['id']))->save(array('token'=>$token_user));
		
		$rs[] = $mo->table('qqbtc_user_log')->add(array('userid' => $user['id'], 'type' => '登录', 'remark' => $remark, 'addtime' => $logintime, 'addip' => $ip, 'addr' => get_city_ip(), 'status' => 1));
	...
	public function password_up($password=""){
		...
		$mo->execute('lock tables qqbtc_user write , qqbtc_user_log write ');
		$rs = array();
		$rs[] = $mo->table('qqbtc_user')->where(array('moble' => $user['moble']))->save(array('password' => md5($password)));
