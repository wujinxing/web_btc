...
class Queue9ce2472db8c3b3d94511365004ce8468Controller extends HomeController
{
	...
	public function checkYichang()
	{
		...
		$mo->execute('lock tables qqbtc_trade write');
		$Trade = M('Trade')->where('deal > num')->order('id desc')->find();

		if ($Trade) {
			if ($Trade['status'] == 0) {
				$mo->table('qqbtc_trade')->where(array('id' => $Trade['id']))->save(array('deal' => Num($Trade['num']), 'status' => 1));
			}
			else {
				$mo->table('qqbtc_trade')->where(array('id' => $Trade['id']))->save(array('deal' => Num($Trade['num'])));
			}
		...
		foreach ($coinList as $k => $v) {
			...
			foreach ($listtransactions as $trans) {
				...
				if ($trans['category'] == 'receive') {
					...
					$rs = array();
					$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $user['id']))->setInc($coin, $trans['amount']);

					if ($res = $mo->table('qqbtc_myzr')->where(array('txid' => $trans['txid']))->find()) {
						echo 'qqbtc_myzr find and set status 1';
						$rs[] = $mo->table('qqbtc_myzr')->save(array('id' => $res['id'], 'addtime' => time(), 'status' => 1));
					}
					else {
						echo 'qqbtc_myzr not find and add a new qqbtc_myzr' . "\n";
						$rs[] = $mo->table('qqbtc_myzr')->add(array('userid' => $user['id'], 'username' => $trans['address'], 'coinname' => $coin, 'fee' => $sfee, 'txid' => $trans['txid'], 'num' => $true_amount, 'mum' => $trans['amount'], 'addtime' => time(), 'status' => 1));
					}
	...
	public function upTrade_qqbtc1111111_8a201aa602cd9448($market = NULL)
	{
		die();
		$userid = rand(1, 2);
		$type = rand(1, 2);
	...
	public function upTrade_qqbtc_8a201aa602cd9448($market = NULL)
	{
		...
		if ($type == 1) {
			$rs[] = $mo->table('qqbtc_trade')->add(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 1, 'addtime' => time(), 'status' => 0));
		}
		else if ($type == 2) {
			$rs[] = $mo->table('qqbtc_trade')->add(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
			$mo->execute('unlock tables');
