		...
		if(WAP_URL !=""){
			...
			if(preg_match('/(iphone|android|Windows\sPhone)/i', $ua)){
				...
				if (isset($_GET['invit'])) {
					...
					if ($user['id']) {
						$qqbtc_redirect = WAP_URL."/Login/register/invit/".$user['id'];
					}else{
						$qqbtc_redirect = WAP_URL;
					}
				}else{
					$qqbtc_redirect = WAP_URL;
				}
				header("Location:".$qqbtc_redirect);
				die();
			} 
		}
		
		//20170511 qqbtc 增加获取币类型函数
		
		//C('coin_menu_qqbtc',array('CNY','BTC','ETH'));
