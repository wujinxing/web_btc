...
class AppController extends AdminController
{
	...
	public function ads_user()
	{
		...
		$where = array();
		$count = $this->Model->join('qqbtc_user ON qqbtc_user.id = qqbtc_app_vipuser.uid')->join('qqbtc_app_vip ON qqbtc_app_vip.id = qqbtc_app_vipuser.vip_id')->field('qqbtc_user.username,qqbtc_app_vipuser.*,qqbtc_app_vip.name as vip_name,qqbtc_app_vip.tag')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = $this->Model->join('qqbtc_user ON qqbtc_user.id = qqbtc_app_vipuser.uid')->join('qqbtc_app_vip ON qqbtc_app_vip.id = qqbtc_app_vipuser.vip_id')->field('qqbtc_user.username,qqbtc_app_vipuser.*,qqbtc_app_vip.name as vip_name,qqbtc_app_vip.tag')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$this->assign('list', $list);
