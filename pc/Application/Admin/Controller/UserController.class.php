...
class UserController extends AdminController
{
	...
	public function edit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if (isset($_POST['id'])) {
                ...
                $rs[] = $mo->table('qqbtc_user')->add($_POST);
	...
	public function admin($name = NULL, $field = NULL, $status = NULL)
	{
		...
		if (!in_array('email', $DbFields)) {
			M()->execute('ALTER TABLE `qqbtc_admin` ADD COLUMN `email` VARCHAR(200)  NOT NULL   COMMENT \'\' AFTER `id`;');
		}
...
/* 		$l_table = $prefix . 'ucenter_member';//Common\Model\AuthGroupModel::MEMBER;
	...
	public function logEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                unset($_POST['id']);
                if (M()->table('qqbtc_user_log')->where(array('id'=>$id))->save($_POST)) {
                    $this->success('编辑成功！');
                } else {
                    $this->error('编辑失败！');
                }
            } else {
                if (M()->table('qqbtc_user_log')->add($_POST)) {
                    $this->success('添加成功！');
                } else {
                    $this->error('添加失败！');
                }
	...
	public function qianbaoEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                unset($_POST['id']);
                if (M()->table('qqbtc_user_qianbao')->where(array('id' => $id))->save($_POST)) {
                    $this->success('编辑成功！');
                } else {
                    $this->error('编辑失败！');
                }
            } else {
                if (M()->table('qqbtc_user_qianbao')->add($_POST)) {
                    $this->success('添加成功！');
                } else {
                    $this->error('添加失败！');
                }
	...
	public function bankEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                if (M()->table('qqbtc_user_bank')->where(array('id' => $id))->save($_POST)) {
                    $this->success('编辑成功！');
                }
                else {
                    $this->error('编辑失败！');
                ...
                if (M()->table('qqbtc_user_bank')->add($_POST)) {
                    $this->success('添加成功！');
                }
                else {
                    $this->error('添加失败！');
	...
	public function coinEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                ...
                if (M()->table('qqbtc_user_coin')->add($_POST)) {
                    $this->success('添加成功！');
                }
                else {
                    $this->error('添加失败！');
	...
	public function goodsEdit($id = NULL)
	{
		if (empty($_POST)) {
            ...
            if ($id) {
                unset($_POST['id']);
                if (M()->table('qqbtc_user_goods')->where(array('id'=>$id))->save($_POST)) {
                    $this->success('编辑成功！');
                }
                else {
                    $this->error('编辑失败！');
                ...
                if (M()->table('qqbtc_user_goods')->add($_POST)) {
                    $this->success('添加成功！');
                }
                else {
                    $this->error('添加失败！');
