...
class ConfigController extends AdminController
{
	...
	public function coinEdit($id = NULL)
	{
		if (empty($_POST)) {
			...
			if ($_POST['id']) {
				...
				$rea = M()->execute('ALTER TABLE  `qqbtc_user_coin` ADD  `' . $_POST['name'] . '` DECIMAL(20,8) UNSIGNED NOT NULL');
				$reb = M()->execute('ALTER TABLE  `qqbtc_user_coin` ADD  `' . $_POST['name'] . 'd` DECIMAL(20,8) UNSIGNED NOT NULL ');
				$rec = M()->execute('ALTER TABLE  `qqbtc_user_coin` ADD  `' . $_POST['name'] . 'b` VARCHAR(200) NOT NULL ');
				
				//对应的商品付款类型增加币种
				$rea = M()->execute('ALTER TABLE  `qqbtc_shop_coin` ADD  `' . $_POST['name'] . '` VARCHAR(200) NOT NULL');
	...
	public function coinStatus()
	{
		...
		case 'delete':
			...
			foreach ($rs as $k => $v) {
				$rs[] = M()->execute('ALTER TABLE  `qqbtc_user_coin` DROP COLUMN ' . $v['name']);
				$rs[] = M()->execute('ALTER TABLE  `qqbtc_user_coin` DROP COLUMN ' . $v['name'] . 'd');
				$rs[] = M()->execute('ALTER TABLE  `qqbtc_user_coin` DROP COLUMN ' . $v['name'] . 'b');
				
				$rs[] = M()->execute('ALTER TABLE  `qqbtc_shop_coin` DROP COLUMN ' . $v['name']);
			}
	...
	public function coinQing($coin)
	{
		...
		$info = M()->execute('UPDATE `qqbtc_user_coin` SET `' . trim($coin) . 'b`=\'\' ;');
	...
	public function checkUpdata()
	{
		...
		if (!S(MODULE_NAME . CONTROLLER_NAME . 'checkUpdata')) {
			...
			if (!in_array('footer_logo', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` ADD COLUMN `footer_logo` VARCHAR(200)  NOT NULL   COMMENT \' \' AFTER `id`;');
			}

			if (in_array('mycz_invit_3', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_invit_3`;');
			}

			if (in_array('mycz_invit_2', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_invit_2`;');
			}

			if (in_array('mycz_invit_1', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_invit_1`;');
			}

			if (in_array('mycz_invit_coin', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_invit_coin`;');
			}

			if (in_array('mycz_fee', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_fee`;');
			}

			if (in_array('mycz_min', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_min`;');
			}

			if (in_array('mycz_max', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_max`;');
			}

			if (in_array('mycz_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_text_index`;');
			}

			if (in_array('mycz_text_log', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mycz_text_log`;');
			}

			if (in_array('mytx_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mytx_text_index`;');
			}

			if (in_array('mytx_text_log', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `mytx_text_log`;');
			}

			if (in_array('trade_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `trade_text_index`;');
			}

			if (in_array('trade_text_entrust', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `trade_text_entrust`;');
			}

			if (in_array('issue_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `issue_text_index`;');
			}

			if (in_array('issue_text_log', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `issue_text_log`;');
			}

			if (in_array('issue_text_plan', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `issue_text_plan`;');
			}

			if (in_array('invit_text_index', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `invit_text_index`;');
			}

			if (in_array('invit_text_award', $DbFields)) {
				M()->execute('ALTER TABLE `qqbtc_config` DROP COLUMN `invit_text_award`;');
			}
			...
			if (!isset($tableMap['qqbtc_daohang'])) {
				M()->execute("\r\n" . '                    CREATE TABLE `qqbtc_daohang` (' . "\r\n" . '                        `id` INT(11) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT \'自增id\',' . "\r\n" . '                        `name` VARCHAR(255) NOT NULL COMMENT \'名称\',' . "\r\n" . '                          `title` VARCHAR(255) NOT NULL COMMENT \'名称\',' . "\r\n" . '                        `url` VARCHAR(255) NOT NULL COMMENT \'url\',' . "\r\n" . '                        `sort` INT(11) UNSIGNED NOT NULL COMMENT \'排序\',' . "\r\n" . '                        `addtime` INT(11) UNSIGNED NOT NULL COMMENT \'添加时间\',' . "\r\n" . '                        `endtime` INT(11) UNSIGNED NOT NULL COMMENT \'编辑时间\',' . "\r\n" . '                        `status` TINYINT(4)  NOT NULL COMMENT \'状态\',' . "\r\n" . '                        PRIMARY KEY (`id`)' . "\r\n\r\n" . '                  )' . "\r\n" . 'COLLATE=\'gbk_chinese_ci\'' . "\r\n" . 'ENGINE=MyISAM' . "\r\n" . 'AUTO_INCREMENT=1' . "\r\n" . ';' . "\r\n\r\n\r\n" . 'INSERT INTO `qqbtc_daohang` (`name`,`title`, `url`, `sort`, `status`) VALUES (\'finance\',\'财务中心\', \'Finance/index\', 1, 1);' . "\r\n" . 'INSERT INTO `qqbtc_daohang` (`name`,`title`, `url`, `sort`, `status`) VALUES (\'user\',\'安全中心\', \'User/index\', 2, 1);' . "\r\n" . 'INSERT INTO `qqbtc_daohang` (`name`, `title`,`url`, `sort`, `status`) VALUES (\'game\',\'应用中心\', \'Game/index\', 3, 1);' . "\r\n" . 'INSERT INTO `qqbtc_daohang` (`name`, `title`,`url`, `sort`, `status`) VALUES (\'article\',\'帮助中心\', \'Article/index\', 4, 1);' . "\r\n\r\n\r\n" . '                ');
			}
