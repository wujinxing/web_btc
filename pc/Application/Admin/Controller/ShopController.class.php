...
class ShopController extends AdminController
{
	...
	public function chexiao($id = NULL)
	{
		...
		$mo->execute('lock tables qqbtc_user_coin write,qqbtc_shop_log write');
		$rs[] = $mo->table('qqbtc_user_coin')->where(array('userid' => $shoplog['userid']))->setInc($shoplog['coinname'], $shoplog['xuyao']);
		$rs[] = $mo->table('qqbtc_shop_log')->where(array('id' => $id))->save(array('status' => 2));
