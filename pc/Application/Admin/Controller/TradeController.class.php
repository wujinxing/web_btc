...
class TradeController extends AdminController
{
	...
	public function market($field = NULL, $name = NULL)
	{
		...
		foreach($list as $k=>$v){
			...
			$list[$k]['tradetimeqqbtc'] = $begintradeqq_3479015851."-".$endtradeqq_3479015851;
		}
	...
	public function marketEdit($id = NULL)
	{
		...
		if (empty($_POST)) {
			if (empty($id)) {
				...
				$market_qqbtc = M('Market')->where(array('id' => $id))->find();
				$this->data = $market_qqbtc;

				if($market_qqbtc['begintrade']){
					$beginshi = explode(":",$market_qqbtc['begintrade'])[0];
					$beginfen = explode(":",$market_qqbtc['begintrade'])[1];
				}else{
					$beginshi = "00";
					$beginfen = "00";
				}
				
				if($market_qqbtc['endtrade']){
					$endshi = explode(":",$market_qqbtc['endtrade'])[0];
					$endfen = explode(":",$market_qqbtc['endtrade'])[1];
				}else{
					$endshi = "23";
					$endfen = "59";
				}
			...
			$this->assign('qqbtc_getCoreConfig',$qqbtc_getCoreConfig['qqbtc_indexcat']);
